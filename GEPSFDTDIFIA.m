...
kx = 1;
ky = 1;
omg = abs(sqrt((kx^2+ky^2)/epsilon/mu)); %% define omg

T = 1;            %% the end time
h1 = 1/40;       %% Coarse Space Step size in X direction with coarse step size h = 1/320
...
tau = 1/80;      % Time Space Step size with step size tau = 1/640
...
fprintf("The relative energy error is %d",abs(W - .25)/.25);
toc
